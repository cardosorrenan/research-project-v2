...
class _PathInitializer(object):
    ...
    def set_interpreter_version(self):
        ...
        if version in _PathInitializer.SUPPORTED_PYTHON_VERSIONS:
            ...
            raise EnvironmentError(
                "Python {0} is not supported.".format(version_with_dot)
            )
    ...
    def __init__(self):
        ...
        self.path_file_name = os.path.join(
            this_folder, "paths.{0}.txt".format(self.arch)
        )
    ...
    def read_path_file(self):
        ...
        if length == 4:
            ...
            if (
                (not os.path.isdir(bin_dir))
                or (not os.path.isdir(extern_bin_dir))
                or (not os.path.isdir(runtime_dir))
                or (not os.path.isdir(ml_dir))
            ):
                return False
            else:
                (self.bin_dir, self.extern_bin_dir, self.runtime_dir, self.ml_dir) = (
                    bin_dir,
                    extern_bin_dir,
                    runtime_dir,
                    ml_dir,
    ...
    def write_path_file(self):
        ...
        if self.system == "Windows":
            print(
                "# extern bin dir: added to both OS path and system path",
                file=path_file,
            )
        else:
    ...
    def get_platform_info(self):
        ...
        if not self.system in _PathInitializer.PLATFORM_DICT:
            ...
            (self.path_var, self.ext, self.lib_prefix) = _PathInitializer.PLATFORM_DICT[
                self.system
            ]
        ...
        if self.system == "Windows":
            ...
            raise RuntimeError(
                "Operating system {0} is not supported.".format(self.system)
            )
    ...
    def get_paths_from_os(self):
        ...
        if self.system == "Windows":
            file_to_find = "".join(
                (
                    self.lib_prefix,
                    "mclmcrrt",
                    _PathInitializer.RUNTIME_VERSION_W_UNDERSCORES,
                    ".",
                    self.ext,
                )
            )
        ...
        if not path_elements:
            if self.system == "Darwin":
                raise RuntimeError(
                    "On the Mac, you must run mwpython rather than python "
        ...
        if not path_found:
            raise RuntimeError(
                "Could not find an appropriate directory for MATLAB or the MATLAB runtime in {0}. Details: {1}".format(
        ...
        if last_path_component != self.arch:
            ...
            raise RuntimeError(
                output_str.format(self.arch, self.path_var, os.sep, path_found)
            )
        ...
        if not os.path.isdir(extern_bin_dir):
            raise RuntimeError(
                "Could not find the directory {0}".format(extern_bin_dir)
            )
    ...
    def import_matlab_pysdk_runtime(self):
        ...
        if not hasattr(self.mr_handle, "_runtime_version_w_dots"):
            raise RuntimeError(
                "Runtime version of package ({0}) does not match runtime version of previously loaded package".format(
                    _PathInitializer.RUNTIME_VERSION_W_DOTS
                )
            )
        elif self.mr_handle._runtime_version_w_dots and (
            self.mr_handle._runtime_version_w_dots
            != _PathInitializer.RUNTIME_VERSION_W_DOTS
        ):
            raise RuntimeError(
                "Runtime version of package ({0}) does not match runtime version of previously loaded package ({1})".format(
                    _PathInitializer.RUNTIME_VERSION_W_DOTS,
                    self.mr_handle._runtime_version_w_dots,
                )
            )
    ...
    def initialize_package(self):
        package_handle = self.mr_handle.DeployablePackage(
            self, self.PACKAGE_NAME, __file__
        )
    ...
    def initialize_runtime(self, option_list):
        if not self.cppext_handle:
            raise RuntimeError(
                "Cannot call initialize_application before import_cppext."
            )
        if self.is_mac:
    ...
    def terminate_runtime(self):
        if not self.cppext_handle:
            raise RuntimeError(
                "Cannot call terminate_application before import_cppext."
            )
        self.cppext_handle.terminateApplication()

    def import_cppext(self):
        self.cppext_handle = importlib.import_module(
            "matlabruntimeforpython" + self.interpreter_version
        )
...
try:
    ...
    print(
        "Exception caught during initialization of Python interface. Details: {0}".format(
            e
        )
    )
    raise
